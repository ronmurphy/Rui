...
impl Cell {
    fn new() -> Self {
        Cell { is_mine: false, is_revealed: false, is_flagged: false, adjacent: 0 }
    }
}
...
impl GameState {
    ...
    fn reveal(&mut self, row: usize, col: usize) -> bool {
        ...
        if self.cells[row][col].is_mine { return true; }
    ...
    fn check_win(&self) -> bool {
        self.cells.iter().flatten().all(|c| c.is_mine || c.is_revealed)
    }
}
            ...
            // Left-click: reveal
            {
                ...
                btn.connect_clicked(move |_| {
                    ...
                    let (hit, won) = {
                        ...
                        if st.cells[r][c].is_revealed || st.cells[r][c].is_flagged { return; }
                        if st.first_click {
            ...
            // Right-click: toggle flag
            {
                ...
                gesture.connect_released(move |_, _, _, _| {
                    let remaining = {
                        ...
                        if cell.is_flagged { st.mines_remaining -= 1; }
                        else               { st.mines_remaining += 1; }
                        st.mines_remaining
                    };
